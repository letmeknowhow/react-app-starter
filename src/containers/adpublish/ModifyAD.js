...
import {getADProductDetail, recordValues, modifyAD, getADList} from '../../actions/adpublish/home';
...
class ModifyAD extends Component {
  ...
  componentWillMount() {
    ...
    //如果当前存储的产品id与路由传入的产品id不同,则重新查询
    if (productId != this.props.state.id) {
      //获取用户信息
      this.getUserInfo();

      //获取广告产品详情
      this.getADDetail(productId);
    }
  }
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <div className="detail-bg">
        <Header leftTo={leftTo} leftIcon={leftIcon} title={'旅游产品发布'} goBackCallback={this.initInput} />
        <div className="release-con">
          <input ref="name" name="name" className="input_style mar-top15" value={name} onChange={this.handleInputChange} />
        ...
        <ul className="product-box">
          <li data-flex>
            ...
            <div className="input-box" data-flex-box="1">
              <select ref="tagType" name="tagType" className="input_style" id="" value={tagType} onChange={this.handleInputChange}>
          ...
          <li data-flex>
            ...
            <div className="input-box" data-flex-box="1"><input ref="price" name="price" className="input_style" type="text" value={price} onChange={this.handleInputChange} /></div>
          </li>
          <li data-flex>
            <div className="product-tit" data-flex-box="0"><div className="table-cell">同业返利</div></div>
            <div className="input-box" data-flex-box="1"><textarea ref="commissionPolicy" name="commissionPolicy" className="text-fields" value={commissionPolicy} onChange={this.handleInputChange}></textarea></div>
          </li>
          <li data-flex>
            <div className="product-tit" data-flex-box="0"><div className="table-cell">产品简介</div></div>
            <div className="input-box" data-flex-box="1"><textarea ref="productPresent" name="productPresent" className="text-fields" value={productPresent} onChange={this.handleInputChange}></textarea></div>
          </li>
          <li data-flex>
  ...
  getADDetail(productID) {
    const {actions} = this.props;
    actions.getADProductDetail(
  ...
  sendData() {
    ...
    //执行action提交广告产品
    this.props.actions.modifyAD(sendData, PAGE_ID);
  }
...
ModifyAD.contextTypes = {
  router: React.PropTypes.object.isRequired
};

export default connect(state =>
    ({state: state.modifyAD}),
  (dispatch) => ({
